...
export default async function KrjPage() {
  ...
  const columns: { key: string; label: string; description: string }[] = [
    ...
    { key: "c", label: "Fri Close", description: `Closing price on last trading day of the week ending ${signalDate}` },
    { key: "weekly_low", label: "Wk Low", description: `Lowest trade price during the week ending ${signalDate}` },
    { key: "25DMA", label: "25 DMA", description: "25-day simple moving average of closing prices" },
    { key: "25DMA_shifted", label: "25DMA -3w", description: "25-day moving average as it was 15 trading days ago; used for stop-loss levels" },
    { key: "long_signal_value", label: "Long Val", description: "(Weekly Low - 25DMA) / 25DMA; positive values indicate strength above the moving average" },
    { key: "short_signal_value", label: "Short Val", description: "(Friday Close - 25DMA) / 25DMA; negative values indicate weakness below the moving average" },
    { key: "signal", label: "Signal", description: "Long if weekly low >= 3% above 25DMA; Short if Friday close <= 3% below 25DMA; otherwise Neutral" },
    { key: "signal_status_prior_week", label: "Prev Wk", description: "Signal status from the prior week" },
    { key: "optimized_signal", label: "Opt. Signal", description: "Current week signal using optimized thresholds (when available)" },
    { key: "optimized_signal_prior_week", label: "Prev Opt", description: "Optimized signal from the prior week (when available)" },
    { key: "25DMA_range_bps", label: "ADR (25d)", description: "25-day average of daily high-low range in basis points" },
    { key: "vol_ratio", label: "Vol Ratio", description: "Stock's average daily range divided by SPY's daily range; measures relative volatility" },
    { key: "market_cap_b", label: "Mkt Cap", description: "Market capitalization (updated weekly)" },
    { key: "25D_ADV_Shares_MM", label: "ADV (MM)", description: "25-day average daily volume in millions of shares" },
    { key: "25D_ADV_nortional_B", label: "ADV ($B)", description: "25-day average daily notional volume in billions of dollars" },
    { key: "avg_trade_size", label: "Avg Trade", description: "Average number of shares per trade (volume / number of trades)" },
