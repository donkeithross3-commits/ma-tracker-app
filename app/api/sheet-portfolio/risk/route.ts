...
export async function GET() {
  try {
    const resp = await fetch(`${PORTFOLIO_SERVICE_URL}/risk/latest`, {
      cache: "no-store",
    });
    if (!resp.ok) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = ticker
      ? `${PORTFOLIO_SERVICE_URL}/risk/assess?ticker=${encodeURIComponent(ticker)}`
      : `${PORTFOLIO_SERVICE_URL}/risk/assess`;
